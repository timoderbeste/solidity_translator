...
def generate_contract(potential_names: [str]):
    ...
    for _ in range(num_components):
        ...
        if component_type == 0:
            components.append(generate_enum('contract', potential_names))
        elif component_type == 1:
            components.append(generate_variable('contract', potential_names))
        elif component_type == 2:
            components.append(generate_function('contract', potential_names))

    return DefineContract(name, components)

def generate_enum(context: str, potential_names: [str]):
    name = get_random_name(potential_names)
    elems = []
    num_elems = random.randint(1, MAX_NUM_ENUM_ELEMS)
    for _ in range(num_elems):
        elem_idx = random.randint(0, len(potential_names) - 1)
        elems.append(potential_names[elem_idx])

    return DefineEnum(context, name, elems)

def generate_variable(context, potential_names: [str], for_func_param=False):
    name = get_random_name(potential_names)

    option_idx = random.randint(0, len(VAR_OPTIONS_SET) - 1)
    options = [VAR_OPTIONS_SET[option_idx]]
    value = generate_expression(potential_names)
    return DefineVariable(None if for_func_param else context, name, options, None if for_func_param else value)

def generate_function(context: str, potential_names: [str]):
    name = get_random_name(potential_names)
    options = [FUNC_OPTIONS_SET[random.randint(0, len(FUNC_OPTIONS_SET) - 1)]]
    params = [generate_variable(None, potential_names, True) for _ in range(MAX_NUM_ARGS)]
    components = get_func_components('function', potential_names)
    return DefineFunction(context, name, options, params, components)

def generate_if_else(context, potential_names):
    bool_cond = generate_equal_exp(potential_names)
    true_stms = get_func_components('if-else-true-statements', potential_names)
    false_stms = get_func_components('if-else-false-statements', potential_names)
    return DefineIfElse(bool_cond, true_stms, false_stms)

def generate_for_loop(context, potential_names):
    ...
    components = get_func_components(None, potential_names)
    return DefineFor(var, bool_cond, increment, components)

def generate_expression(potential_names, for_num_operation=False):
    while True:
        exp_type = random.randint(0, NUM_POSSIBLE_EXPS - 1)
        if exp_type == 0:
            return generate_call_exp(potential_names)
        elif exp_type == 1:
            return generate_variable_exp(potential_names)
        elif exp_type == 2:
            return generate_number_exp()
        elif exp_type == 3 and not for_num_operation:
            return generate_boolean_exp()
        elif exp_type == 4:
            return generate_multiply_exp(potential_names)
        elif exp_type == 5 and not for_num_operation:
            return generate_equal_exp(potential_names)
        elif exp_type == 6 and not for_num_operation:
            return generate_enum_exp(potential_names)

def generate_call_exp(potential_names):
    name = get_random_name(potential_names)
    num_args = random.randint(0, MAX_NUM_ARGS)
    args = []
    for _ in range(num_args):
        args.append(generate_expression(potential_names))
    return Call(name, args)

def generate_variable_exp(potential_names):
...
def generate_multiply_exp(potential_names):
    exp1 = generate_expression(potential_names, for_num_operation=True)
    exp2 = generate_expression(potential_names, for_num_operation=True)
    return Multiply(exp1, exp2)

def generate_equal_exp(potential_names):
    exp1 = generate_expression(potential_names)
    exp2 = generate_expression(potential_names)
    return Equal(exp1, exp2)

def generate_enum_exp(potential_names):
...
def get_func_components(context, potential_names):
    components = []
    num_components = random.randint(1, MAX_NUM_COMPONENTS)
    for _ in range(num_components):
        component_type = random.randint(0, NUM_POSSIBLE_FUNC_COMPONENTS - 1)
        if component_type == 0:
            components.append(generate_enum(context, potential_names))
        elif component_type == 1:
            components.append(generate_variable(context, potential_names))
        elif component_type == 2:
            components.append(generate_if_else(context, potential_names))
        elif component_type == 3:
            components.append(generate_for_loop(context, potential_names))
    return components